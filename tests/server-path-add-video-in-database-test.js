...
describe('Path: Add video in database', () => {
    beforeAll(() => {
        helpers.deleteFile(
            './tests/resources/db-tests-for-modification-01.temp'
        );
        helpers.deleteFile(
            './tests/resources/db-tests-for-modification-02.temp'
        );
        helpers.deleteFile(
            './tests/resources/db-tests-for-modification-03.temp'
        );
        helpers.deleteFile(
            './tests/resources/db-tests-for-modification-04.temp'
        );
    ...
    test('Add new video', () => {
        ...
        expect(
            dbTests.get('videos').filter({ videoId: 'video-01' }).value()[0]
                .videoTitle
        ).toBe('Video 01');
        expect(dbTests.get('channels').size().value()).toBe(1);
        expect(
            dbTests.get('channels').filter({ channelId: channelId }).value()[0]
                .channelTitle
        ).toBe('Channel 01');
        ...
        expect(
            dbTests.get('videos').filter({ videoId: videoIdToAdd }).value()[0]
                .videoTitle
        ).toBe(videoTitleToAdd);
        expect(
            dbTests.get('videos').filter({ videoId: videoIdToAdd }).value()[0]
                .videoDuration
        ).toBe(4320);
    ...
    test('Add new video from new channel', () => {
        ...
        const request = {
            body: {
                ...
                videoId: 'video-03',
                videoTitle: 'Video 03',
                channelTitle: channelTitleToAdd,
            },
        };
        const response = mockResponse();
        ...
        expect(
            dbTests
                .get('channels')
                .filter({ channelId: channelIdToAdd })
                .value()[0].channelTitle
        ).toBe(channelTitleToAdd);
    });
