...
async function openDB() {
    ...
    fs.copyFileSync(
        './tests/resources/db-zhykos.json',
        './tests/resources/db-zhykos.temp'
    );
    ...
    jest.spyOn(vcfServer.db, 'get').mockImplementation((getWhat) =>
        dbTests.get(getWhat)
    );
    jest.spyOn(vcfServer.db, 'has').mockImplementation((getWhat) =>
        dbTests.has(getWhat)
    );
}
...
describe('Selenium tests', () => {
    ...
    test('Display hidden videos', async () => {
        ...
        const linkHideHidden3 = await helpers.selectId(
            'button-hide-hidden',
            driver
        );
    ...
    test('Change channel', async () => {
        ...
        const linkZhykos = await helpers.selectId(
            'button-channel-UCWJHqzXc7rKO7h5TzYYBNFw',
            driver
        );
        ...
        const linkSkywilly = await helpers.selectId(
            'button-channel-UCnygly7mB_KSETq9zPclZVA',
            driver
        );
        ...
        const linkAllChannels = await helpers.selectId(
            'button-channel-#all',
            driver
        );
    ...
    test('Open video iframe', async () => {
        ...
        const linkOpen = await helpers.selectId(
            'button-open-4QXpyrp8WUo',
            driver
        );
    ...
    test('Show masked video', async () => {
        ...
        const linkHideHidden = await helpers.selectId(
            'button-hide-hidden',
            driver
        );
    ...
    test.skip('Open video in source website', async () => {
        ...
        const linkOpenOrigin = await helpers.selectId(
            'button-open-website-FK30dDJh7fQ',
            driver
        );
    ...
    test.skip('Check CSS', async () => {
        ...
        await helpers.assertAttributeById(
            'swap_LOilCuZhB5o',
            'src',
            rootURL + 'images/visible.png',
            driver
        );
        await helpers.assertAttributeById(
            'swap_ATsjtKOX-qY',
            'src',
            rootURL + 'images/hide.png',
            driver
        );
    });
});
...
describe('Empty database', () => {
    ...
    test('empty homepage', async () => {
        ...
        if (
            ...
            config.SCREENSHOTS_TESTS == 'true'
        ) {
            helpers.takeScreenshotForDocumentation(
                'client-guide-empty-temp',
                driver
            );
